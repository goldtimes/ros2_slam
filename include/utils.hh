   ...
   public:
    ...
    static void Evaluate(F&& func, const std::string& func_name) {
        ...
        if (records_.find(func_name) != records_.end()) {
            ...
            LOG_INFO("{} time:{}", func_name, time_used);
        } else {
    ...
    static void PrintAll() {
        ...
        for (const auto& r : records_) {
            // LOG_INFO(INFO) << "> [ " << r.first << " ] average time usage: "
            //                << std::accumulate(r.second.time_usage_in_ms_.begin(), r.second.time_usage_in_ms_.end(),
            //                                0.0) /
            //                       double(r.second.time_usage_in_ms_.size())
            //                << " ms , called times: " << r.second.time_usage_in_ms_.size();
        }
        LOG_INFO("===== Printing run time end =====");
    }
