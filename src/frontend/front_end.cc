...
FrontEnd::FrontEnd(System *system) : system_(system) {
  ...
  if (use_encoder_) {
    encoder_processor_ptr_ =
        std::make_shared<EncoderProcessor>(system_->GetSystemConfig());
    encoder_processor_ptr_->SetTransformWheelToImu(T_EI);
    kf_ptr_->SetWheelLossFunc([&](const Encoder &encoder, const Input &input,
                                  State &x, ESKFShareState &share_state) {
      encoder_processor_ptr_->UpdateEncoder(encoder, input, x, share_state);
    });
    // 设置encoder外参
    // 保证旋转的正交性，因为sophus对矩阵要求很严格 1 0 -2.77515e-17 0 1 0 0
    // -2.77515e-17  0  1 这样的矩阵会报错
    kf_ptr_->x().rot_R_IE =
        Eigen::Quaterniond(T_EI.inverse().R).toRotationMatrix();
    kf_ptr_->x().pos_t_IE = T_EI.inverse().t;
  }

  if (use_gnss_) {
    kf_ptr_->SetGnssLossFunc(
        [&](const GNSS &gnss, State &x, ESKFShareState &share_state) {
          UpdateGnss(gnss, x, share_state);
        });
  }

  // propogator
  propogator_ptr_ =
      std::make_shared<Propogator>(system_->GetSystemConfig(), kf_ptr_);
  propogator_ptr_->SetTransformWheelToImu(T_EI);
  AllocateMemory();
  // voxel_map_odom
  if (system_->GetSystemConfig()->use_voxel_) {
    lidar_register_ptr_ =
        std::make_shared<VoxelMapRegister>(system_->GetSystemConfig(), kf_ptr_);
  } else if (system_->GetSystemConfig()->use_p2plane_) {
    lidar_register_ptr_ =
        std::make_shared<P2PlaneRegister>(system_->GetSystemConfig(), kf_ptr_);
  } else if (system_->GetSystemConfig()->use_ndt_) {
    lidar_register_ptr_ =
        std::make_shared<IncNdtRegister>(system_->GetSystemConfig(), kf_ptr_);
  } else if (system_->GetSystemConfig()->use_fasterlio_) {
    lidar_register_ptr_ = std::make_shared<FasterlioRegister>(
        system_->GetSystemConfig(), kf_ptr_);
  } else {
  ...
  if (use_pgo_) {
    pose_graph_optimizer_ = std::make_shared<PoseGraphOptimizer>(
        system_->GetSystemConfig()->frontend_config_.pose_graph_config,
        system_->GetSystemConfig()->frontend_config_.loop_closure_config);
    LOG_INFO(GREEN "Pose graph optimization enabled!" RESET);
  }
...
void FrontEnd::Run() {
  ...
  while (ros::ok()) {
    ...
    if (system_->m_buff_cv_.wait_for(lock, std::chrono::milliseconds(1000),
                                     [&] {
                                       // LOG_INFO("lidar_queue size: {}",
                                       // system_->lidar_queue_.size());
      ...
      if (GetMeasureGroup(meas)) {
        ...
        PoseTrans start_pose =
            PoseTrans(kf_ptr_->GetState().rot, kf_ptr_->GetState().pos);
        evaluate_and_call([&]() { propogator_ptr_->PropogateState(meas); },
                          "propogate_and_undistort", false);
        PoseTrans end_pose =
            PoseTrans(kf_ptr_->GetState().rot, kf_ptr_->GetState().pos);
        PoseTrans state_delta_pose = end_pose * start_pose.inverse();
        LOG_INFO("state_delta_pose t_norm: {}, rotation_norm:{}",
                 state_delta_pose.t.norm(), state_delta_pose.RPY().norm());
        // 对轮速计进行积分，对首尾进行插值
        if (use_encoder_) {
          encoder_processor_ptr_->AddEncoder(measure_group_.encoders);
          PoseTrans encoder_delta_pose;
          auto res = encoder_processor_ptr_->Propagation(
              encoder_delta_pose, measure_group_.lidar_beg_time,
              measure_group_.lidar_end_time);
          if (res) {
            LOG_INFO("encoder_delta_pose t_norm: {}, rotation_norm:{}",
                     encoder_delta_pose.t.norm(),
        ...
        undistort_cloud_robot_ =
            TransformLidarOMP(undistort_cloud_lidar_, T_BL.R, T_BL.t);
        // transform to world
        ...
        undistort_cloud_odom_ =
            TransformLidarOMP(undistort_cloud_lidar_, T_WL.R, T_WL.t);
        ...
        if (front_end_status_ == FrontEndStatus::MAPPING) {
          ...
          if (lidar_register_ptr_->Align(undistort_cloud_lidar_, kf_ptr_)) {
              ...
              if (use_loop_closure_ && lidar_register_ptr_->IsKeyFrame()) {
                ...
                if (kf_cloud->size() > 100) {
                  ...
                  if (loop_closure_detector_->DetectLoop(result)) {
                    LOG_INFO(GREEN "Loop detected! {} <-> {}" RESET,
                             result.current_keyframe_id,
                             result.match_keyframe_id);
                  }
                }
              }

              // PGO 位姿图优化
              if (use_pgo_ && lidar_register_ptr_->IsKeyFrame()) {
                PoseTrans kf_pose =
                    lidar_register_ptr_->GetCurrentKeyframePose();
                PointCloudXYZIPtr kf_cloud =
                    lidar_register_ptr_->GetCurrentKeyframeCloud();
                if (kf_cloud->size() > 100) {
                  pose_graph_optimizer_->AddKeyframe(
                      keyframe_count_, kf_pose, kf_cloud,
                      measure_group_.lidar_beg_time);
                }
              }
            ...
            system_->GetLocalizer()->SetLidarCloud(undistort_cloud_robot_,
                                                   T_RtoO);
            // auto t4 = std::chrono::high_resolution_clock::now();
            // auto total_time =
            // std::chrono::duration_cast<std::chrono::duration<double>>(t4 -
            // t3).count(); LOG_INFO("Get Map used time: {} ms", total_time *
            // 1e3);
            ...
            // t2).count(); LOG_INFO("Update Map used time: {} ms", total_time *
            // 1e3);
          } else {
...
bool FrontEnd::GetMeasureGroup(MeasureGroup &measures) {
  // 如果队列为空
  if (system_->imu_queue_.empty() || system_->lidar_queue_.empty() ||
      system_->lidar_time_queue_.empty()) {
    return false;
  }
  // 使用轮速计，但是轮速计的队列为空
  ...
  if (!lidar_pushed_) {
    ...
    // sort cloud
    std::sort(
        measures.curent_cloud->points.begin(),
        measures.curent_cloud->points.end(),
    ...
    if (measures.curent_cloud->size() < 1) {
      ...
      LOG_ERROR("lidar cloud size is 0, begin time is {}, end time is {}",
                measures.lidar_beg_time, measures.lidar_end_time);
    } else if (measures.curent_cloud->points.back().time <
               0.5 * lidar_mean_scantime_) {
      measures.lidar_end_time = measures.lidar_beg_time + lidar_mean_scantime_;
      LOG_ERROR(
          "lidar cloud end time is too small, begin time is {}, end time is {}",
          measures.lidar_beg_time, measures.lidar_end_time);
    } else {
      scan_count_++;
      measures.lidar_end_time = measures.curent_cloud->points.back().time;
      lidar_mean_scantime_ += (measures.lidar_end_time -
                               measures.lidar_beg_time - lidar_mean_scantime_) /
                              scan_count_;
    }
    if (lidar_mean_scantime_ > 0.2) {
      LOG_WARN("lidar mean scan time is too large, mean scan time is {}",
               lidar_mean_scantime_);
    }
    lidar_pushed_ = true;
    LOG_INFO("lidar cloud size is {}, begin time is {:03.6f}, end time is "
             "{:03.6f}, mean scan time is {:03.6f}",
             measures.curent_cloud->size(), measures.lidar_beg_time,
             measures.lidar_end_time, lidar_mean_scantime_);
  }
  // 处理imu数据
  ...
  if (!measures.imus.empty()) {
    LOG_DEBUG("imu size is {}, imu begin_time {:03.3f}, imu_end_time {:03.3f}",
              measures.imus.size(), measures.imus.front().timestamp_,
              measures.imus.end()->timestamp_);
  }
  // 处理encoder数据
  ...
  if (use_encoder_) {
    ...
    if (!measures.encoders.empty()) {
      LOG_DEBUG(
          "encoder size is {}, encoder begin_time {}, encoder_end_time {}",
          measures.encoders.size(), measures.encoders.front().timestamp_,
          measures.encoders.back().timestamp_);
    }
  }
  // 处理gnss数据
  double gnss_time = system_->gnss_queue_.front().timestamp_;
  if (use_gnss_) {
    while (!system_->gnss_queue_.empty() &&
           gnss_time < measures.lidar_end_time) {
    ...
    if (!measures.gnsss.empty()) {
      LOG_INFO(
          "gnss size is {}, gnss begin_time {:03.3f}, gnss_end_time {:03.3f}",
          measures.gnsss.size(), measures.gnsss.front().timestamp_,
          measures.gnsss.back().timestamp_);
    }
  }
...
void FrontEnd::UpdateGnss(const GNSS &gnss, State &state,
  ...
  // Eigen::JacobiSVD<Eigen::MatrixXd> svd(gnss_cov, Eigen::ComputeThinU |
  // Eigen::ComputeThinV); double tolerance = 1e-6;  // 奇异值阈值
...
State FrontEnd::GetCurentNavState() { return kf_ptr_->GetState(); }
...
const M33D FrontEnd::GetCov() const { return kf_ptr_->GetCov(); }
