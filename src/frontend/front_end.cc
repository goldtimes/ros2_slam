...
FrontEnd::FrontEnd(System *system) : system_(system) {
    ...
    if (system_->GetSystemConfig()->use_voxel_) {
        ...
        lidar_register_ptr_ = std::make_shared<SuperLIORegister>(system_->GetSystemConfig(), kf_ptr_);
    } else {
...
bool FrontEnd::GetMeasureGroup(MeasureGroup &measures) {
    ...
    if (!lidar_pushed_) {
        ...
        lidar_pushed_ = true;
        LOG_INFO(
            "lidar cloud size is {}, begin time is {:03.6f}, end time is "
