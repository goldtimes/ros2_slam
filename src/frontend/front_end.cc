...
#include "lidar_register/superlio_register.hh"
...
FrontEnd::FrontEnd(System *system) : system_(system) {
  ...
  } else if (system_->GetSystemConfig()->use_superlio_) {
    lidar_register_ptr_ =
        std::make_shared<SuperLIORegister>(system_->GetSystemConfig(), kf_ptr_);
  } else {
  ...
  while (ros::ok()) {
    ...
    if (system_->m_buff_cv_.wait_for(lock, std::chrono::milliseconds(1000),
      ...
      if (GetMeasureGroup(meas)) {
        ...
        undistort_cloud_body_ =
            TransformLidarOMP(undistort_cloud_lidar_, T_IL.R, T_IL.t);
        // transform to world
        ...
        if (front_end_status_ == FrontEndStatus::MAPPING) {
          ...
          if (lidar_register_ptr_->Align(undistort_cloud_lidar_, kf_ptr_)) {
            ...
            if (lidar_register_ptr_->IsKeyFrame() || first_frame_) {
              ...
              if (system_->GetSlamMode() == SLAM_MODE::LOCALIZATION) {
