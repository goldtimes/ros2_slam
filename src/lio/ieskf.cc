...
int IESKF::P_ID = 0, IESKF::R_ID = 3, IESKF::ER_ID = 6, IESKF::EP_ID = 9,
    IESKF::V_ID = 12, IESKF::BG_ID = 15, IESKF::BA_ID = 18, IESKF::G_ID = 21;

M3D IESKF::Jr(const V3D &inp) { return Sophus::SO3d::jl(inp).transpose(); }
...
void IESKF::UpdateLidar() {
  ...
  for (size_t i = 0; i < max_iter_num_; i++) {
    ...
    // auto time = std::chrono::duration_cast<std::chrono::duration<double>>(t2
    // - t1).count(); LOG_INFO("lidar loss time:{}", time * 1e3);
    ...
    delta = -H_.inverse() * b_;
    // LOG_INFO("delta_norm r:{}, t:{}", delta.segment<3>(0).norm(),
    // delta.segment<3>(3).norm()); if (delta.segment<3>(0).norm() > 0.1 ||
    // delta.segment<3>(3).norm() > 0.1) {
    //     break;
    // }
