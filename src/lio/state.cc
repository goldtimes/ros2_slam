...
V33D State::operator-(const State &other) {
  ...
  if (v_sin < 1e-11) {
    ...
    res = theta / v_sin * other.getBx().transpose() *
          Sophus::SO3d::hat(other.g) * g;
  }
  delta.segment<2>(21) = res;
  delta.segment<3>(23) =
      Sophus::SO3d(other.rot_R_IE.transpose() * rot_R_IE).log();
  delta.segment<3>(26) = pos_t_IE - other.pos_t_IE;
  delta.segment<1>(29) = wheel_scale - other.wheel_scale;
  delta.segment<3>(30) =
      Sophus::SO3d(other.rot_R_IG.transpose() * rot_R_IG).log();
  return delta;
}

Matrix3x2d State::getBx() const {
  Matrix3x2d res;
  res << -g[1], -g[2], GRAVITY - g[1] * g[1] / (GRAVITY + g[0]),
      -g[2] * g[1] / (GRAVITY + g[0]), -g[2] * g[1] / (GRAVITY + g[0]),
      GRAVITY - g[2] * g[2] / (GRAVITY + g[0]);
  res /= GRAVITY;
  return res;
}

Matrix3x2d State::getMx() const { return -Sophus::SO3d::hat(g) * getBx(); }

Matrix3x2d State::getMx(const Eigen::Vector2d &res) const {
  Matrix3x2d bx = getBx();
  Eigen::Vector3d bu = bx * res;
  //   std::cout << "bu: " << bu.transpose() << std::endl;
  return -Sophus::SO3d::exp(bu).matrix() * Sophus::SO3d::hat(g) *
         Sophus::SO3d::jl(bu).transpose() * bx;
}
