...
PosegraphOptimization::PosegraphOptimization(ros::NodeHandle &nh) : nh_(nh) {
  ...
  // 回环检测线程启动
  loopdetection_thread_ =
      std::thread(&PosegraphOptimization::runLoopDetection, this);
  // 回环约束线程启动
  loopconstraint_thread_ =
      std::thread(&PosegraphOptimization::runLoopConstraint, this);
  // isam优化线程启动
  isam_update_thread_ =
      std::thread(&PosegraphOptimization::runISAMUpdate, this);
  // 地图可视化线程启动
  map_visualization_thread_ =
      std::thread(&PosegraphOptimization::runMapVisualization, this);
}
...
void PosegraphOptimization::initNoise() {
  ...
  robustLoopNoise = gtsam::noiseModel::Robust::Create(
      gtsam::noiseModel::mEstimator::Cauchy::Create(
          1), // optional: replacing Cauchy by DCS or GemanMcClure is okay
...
void PosegraphOptimization::init_subpub() {
  ...
  pubLoopScanLocal =
      nh_.advertise<sensor_msgs::PointCloud2>("loop_scan_local", 100);
  pubLoopSubmapLocal =
      nh_.advertise<sensor_msgs::PointCloud2>("loop_submap_local", 100);
  pubLoopScanLocalRegisted =
      nh_.advertise<sensor_msgs::PointCloud2>("loop_scan_local_registed", 100);
  ...
  pubLoopConstraintEdge =
      nh_.advertise<visualization_msgs::MarkerArray>("loop_closure_edges", 100);
...
void PosegraphOptimization::run() {
  while (ros::ok()) {
    ...
    while (!odomBuf.empty() && !cloudBuf.empty()) {
      ...
      // 存储关键点云和位姿
      // 对关键点云进行滤波
      PointCloudXYZIPtr filteredKeyframe =
          VoxelFilter(thisKeyframe, keyframe_downsample);
      // 加锁
      mKF.lock();
      ...
      if (!gtSAMgraphMade) {
        ...
        gtSAMgraph.add(gtsam::BetweenFactor<gtsam::Pose3>(
            prev_node_idx, curr_node_idx, relativePose, odometryNoise));
        // TODO GPS factor
  ...
  while (ros::ok()) {
    ...
    while (!loopClosureQueue.empty()) {
      if (loopClosureQueue.size() > 30) {
        LOG_WARN("Too many loop clousre candidates to be ICPed is waiting ... "
      ...
      if (relativePose3.equals(gtsam::Pose3::Identity())) {
        mGraph.lock();
        gtSAMgraph.add(gtsam::BetweenFactor<gtsam::Pose3>(
            curr_node_idx, prev_node_idx, relativePose3, robustLoopNoise));
        mGraph.unlock();
      }
...
// icp 计算相对位姿
PoseTrans PosegraphOptimization::doICPVirtualRelative(int loopKeyPre,
                                                      int loopKeyCur) {
  ...
  pcl::PointCloud<PointXYZI>::Ptr unused_result(
  ...
  if (icp.hasConverged() == false ||
    ...
    LOG_INFO("[SC loop] ICP fitness test passed ({} < {}). Add this SC loop.",
             icp.getFitnessScore(), loopFitnessScoreThreshold);
  }
  ...
  auto opt_after_pose =
      PoseTrans(icp_transform.block<3, 3>(0, 0).cast<double>(),
                icp_transform.block<3, 1>(0, 3).cast<double>()) *
...
void PosegraphOptimization::loopFindNearKeyframe(
    PointCloudXYZIPtr &nearKeyframes, const int key, const int search_num) {
  nearKeyframes->clear();
  ...
  for (int i = -search_num; i <= search_num; ++i) {
    ...
    TransformCloud(kfcloud_inLidar, kfcloud_inMap, kfPose.pose.R,
                   kfPose.pose.t);
...
void PosegraphOptimization::updatePose() {
  mKF.lock();
  for (int node_idx = 0;
       node_idx < static_cast<int>(isamCurrentEstimate.size()); node_idx++) {
  ...
  const gtsam::Pose3 &latestOptimizedPose =
      isamCurrentEstimate.at<gtsam::Pose3>(
          static_cast<int>(isamCurrentEstimate.size()) - 1);
...
void PosegraphOptimization::performRSLoopClosure() {
  ...
  if (detectLoopClosureDistance(loopKeyCur, loopKeyPre)) {
    LOG_INFO("Loop closure detected between keyframes {} and {}", loopKeyCur,
             loopKeyPre);
    mBuf.lock();
...
bool PosegraphOptimization::detectLoopClosureDistance(int &loopKeyCur,
                                                      int &loopKeyPre) {
  ...
  keyframePoseKdTree.radiusSearch(keyframePoseCloud->back(),
                                  historyKeyframeSearchRadius,
                                  pointSearchIdxLoop, pointSearchSqDisLoop, 0);
  // 遍历被搜索到的点，判断是否有回环
  for (int i = 0; i < pointSearchIdxLoop.size(); i++) {
    int id = pointSearchIdxLoop[i];
    // 时间差满足条件，说明找到时间上比较老的关键帧了，认为是回环
    if (std::abs(keyframeTimeBuf[id] - keyframeTimeBuf[loopKeyCur]) >
...
void PosegraphOptimization::addKFPoseToCloud(const PoseTrans &pose) {
  keyframePoseCloud->points.push_back(
      pcl::PointXYZ(pose.t.x(), pose.t.y(), pose.t.z()));
}
...
void PosegraphOptimization::visualizeLoopClosure() {
  ...
  markerNode.header.stamp =
      ros::Time().fromSec(keyframeTimeBuf[keyframePoseIds.size() - 1]);
...
gtsam::Pose3 PosegraphOptimization::poseTransToPose3(const PoseTrans &pose) {
  return gtsam::Pose3(
      gtsam::Rot3::RzRyRx(pose.RPY().x(), pose.RPY().y(), pose.RPY().z()),
...
void PosegraphOptimization::publishMap() {
  ...
  for (int node_idx = 0; node_idx < recentIdxUpdated; node_idx++) {
    if (counter % SKIP_FRAMES == 0) {
      ...
      TransformCloud(keyframeCloudBuf.at(node_idx), tmpCloud,
                     keyframePoseOptimized.at(node_idx).pose.R,
...
void PosegraphOptimization::publishCloud(ros::Publisher &pub,
