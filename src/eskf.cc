...
void ESKF::Predict(const double& dt, const ProcessNoiseType& Q, const V3D& gyro, const V3D& acce) {
    Eigen::Matrix<double, 24, 1> f_ = x_.get_f(gyro, acce);  // 调用get_f 获取 速度 角速度 加速度
    // 这里并没有完全计算完fx,没有旋转部分计算
    ...
    for (auto st : x_.SO3_states_) {
        ...
        F_x1_.block<3, 3>(idx, idx) = exp(seg_SO3, 0.5).matrix();
    ...
    for (auto st : x_.S2_states_) {
        ...
        res_temp_S2 = -Nx * res.matrix() * x_before_hat * A_matrix(seg_S2).transpose();
...
void ESKF::Update(ESKF::ObsType obs, const double& R) {
    ...
    for (int i = -1; i < maximum_iter_; i++) {
        ...
        for (auto it : x_.SO3_states_) {
            int idx = it.idx_;
            V3D seg_SO3 = dx.block<3, 1>(idx, 0);
            M3D res_temp_SO3 = A_matrix(seg_SO3).transpose();  // 小块的J阵, SO3上的雅可比？
