...
public:
  explicit SpdLogger(const Config config = Config()) : config_(config) {
    ...
    LOG_INFO("Logger console level: {}",
             spdlog::level::to_string_view(config_.console_level));
    LOG_INFO("Logger file level: {}",
             spdlog::level::to_string_view(config_.file_level));
  }
...
private:
  ...
  void setup_logger() {
    ...
    logger_ = std::make_shared<spdlog::async_logger>(
        config_.name, sinks.begin(), sinks.end(), spdlog::thread_pool(),
  ...
  void get_env_level(const char *env_var,
    ...
    std::unordered_map<std::string, spdlog::level::level_enum> level_map = {
        ...
        {"TRACE", spdlog::level::trace}, {"ERROR", spdlog::level::err},
        {"WARN", spdlog::level::warn},   {"CRITICAL", spdlog::level::critical},
    };
    if (!level_str) {
      return;
    }
