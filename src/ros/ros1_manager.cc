...
ROS1Manager::ROS1Manager(const ros::NodeHandle &nh,
  ...
  voxel_map_timer_ = nh_.createTimer(
      ros::Duration(5.0), &ROS1Manager::voxelTimerCB, this, false, false);
  if (system_ptr_->GetSystemConfig()
...
void ROS1Manager::InitPub() {
  ...
  cloud_lidar_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("/lie_slam/lidar", 10);
  cloud_robot_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("/lie_slam/robot_lidar", 10);
  cloud_odom_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("/lie_slam/world_lidar", 10);
  ...
  if (has_encoder_) {
    encoder_path_pub_ =
        nh_.advertise<nav_msgs::Path>("/lie_slam/encoder_path", 10);
  }
  if (has_gnss_) {
    gnss_path_pub_ = nh_.advertise<nav_msgs::Path>("/lie_slam/gnss_path", 10);
    gnss_odom_pub_ =
        nh_.advertise<nav_msgs::Odometry>("/lie_slam/gnss_odom", 10);
  }
  global_map_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("/lie_slam/global_map", 1, true);
  submap_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("/lie_slam/submap", 1, true);
  loop_closure_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "/lie_slam/loop_closure", 1, true);
}

void ROS1Manager::InitSub() {
  imu_sub_ = nh_.subscribe(
      system_ptr_->GetSystemConfig()->imu_config_.imu_topic, 100,
      &ROS1Manager::ImuCallback, this, ros::TransportHints().tcpNoDelay());
  if (system_ptr_->GetSystemConfig()->lidar_config_.use_livox_driver == 0) {
    lidar_sub_ =
        nh_.subscribe(system_ptr_->GetSystemConfig()->lidar_config_.lidar_topic,
                      10, &ROS1Manager::StandarCloudCallback, this,
                      ros::TransportHints().tcpNoDelay());
    LOG_INFO("use standard lidar driver");
  } else if (system_ptr_->GetSystemConfig()->lidar_config_.use_livox_driver ==
             1) {
    lidar_sub_ =
        nh_.subscribe(system_ptr_->GetSystemConfig()->lidar_config_.lidar_topic,
                      10, &ROS1Manager::LivoxCloudCallback, this,
                      ros::TransportHints().tcpNoDelay());
    ...
    lidar_sub_ =
        nh_.subscribe(system_ptr_->GetSystemConfig()->lidar_config_.lidar_topic,
                      10, &ROS1Manager::Livox2CloudCallback, this,
                      ros::TransportHints().tcpNoDelay());
  ...
  encoder_sub_ = nh_.subscribe(
      system_ptr_->GetSystemConfig()->encoder_config_.encoder_topic, 100,
      &ROS1Manager::EncoderCallback, this, ros::TransportHints().tcpNoDelay());
  if (has_gnss_) {
    gnss_sub_ = nh_.subscribe(
        system_ptr_->GetSystemConfig()->gnss_config_.gnss_topic, 100,
  ...
  if (system_ptr_->GetSystemConfig()->localizer_config_.use_meta_maps) {
    metamaps_sub_ =
        nh_.subscribe("metaset_info", 1, &ROS1Manager::MetamapsCallback, this);
  }

  ros_init_pose_sub_ =
      nh_.subscribe("/initialpose", 1, &ROS1Manager::RosInitPoseCallback, this);
  init_pose_sub_ =
      nh_.subscribe("/init_pose", 1, &ROS1Manager::InitPoseCallback, this);
}
void ROS1Manager::InitService() {}

void ROS1Manager::StandarCloudCallback(
  ...
  if (last_lidar_time_ != -1 && curr_lidar_time - last_lidar_time_ > 0.2) {
    ...
    LOG_ERROR("current_lidar_time:{}, last_lidar_time:{}", curr_lidar_time,
              last_lidar_time_);
  }
  ...
  evaluate_and_call(
      [&]() { system_ptr_->GetLidarProcess()->Process(cloud_msg, cloud_ptr); },
      "lidar_process");
...
void ROS1Manager::Livox2CloudCallback(
  ...
  evaluate_and_call(
      [&]() {
        system_ptr_->GetLidarProcess()->Process(cloud_livox, cloud_ptr);
      },
      "lidar_process");
  // push to system
  system_ptr_->AddLidar(cloud_ptr, curr_lidar_time);
}
void ROS1Manager::LivoxCloudCallback(
    const livox_ros_driver::CustomMsg::ConstPtr &cloud_livox) {
  static double last_record_lidar_time = cloud_livox->header.stamp.toSec();
  double curr_lidar_time = cloud_livox->header.stamp.toSec();
  if (curr_lidar_time - last_record_lidar_time >= 5.0) {
    lidar_fps_ = static_cast<int>(lidar_frame_count_ / 5);
...
void ROS1Manager::ImuCallback(const sensor_msgs::Imu::ConstPtr &imu_msg) {
  ...
  if (last_imu_time_ != -1 && curr_imu_time - last_imu_time_ > 0.2) {
    ...
    LOG_ERROR("current_imu_time:{}, last_imu_time:{}", curr_imu_time,
              last_imu_time_);
  }
  ...
  acc << imu_msg->linear_acceleration.x *
             system_ptr_->GetSystemConfig()->imu_config_.imu_scale,
...
void ROS1Manager::EncoderCallback(
  ...
  if (last_encoder_time_ != -1 &&
      curr_encoder_time - last_encoder_time_ > 0.2) {
    LOG_ERROR("encoder data lost");
    LOG_ERROR("current_encoder_time:{}, last_encoder_time:{}",
              curr_encoder_time, last_encoder_time_);
  }
  last_encoder_time_ = curr_encoder_time;
  encoder_frame_count_++;
  V3D linear_vel, angular_vel;
  linear_vel << encoder_msg->twist.twist.linear.x,
      encoder_msg->twist.twist.linear.y, encoder_msg->twist.twist.linear.z;
  angular_vel << encoder_msg->twist.twist.angular.x,
      encoder_msg->twist.twist.angular.y, encoder_msg->twist.twist.angular.z;
...
void ROS1Manager::GNSSCallback(
  ...
  if (last_gnss_time_ != -1 && curr_gnss_time - last_gnss_time_ > 2) {
    ...
    LOG_ERROR("current_gnss_time:{}, last_gnss_time:{}", curr_gnss_time,
              last_gnss_time_);
  }
  ...
  if (!gnss_init_) {
    ...
    V3D pos_cov =
        V3D(gnss_msg->position_covariance[0], gnss_msg->position_covariance[4],
...
void ROS1Manager::Visualize() {
  ...
  while (ros::ok()) {
    ...
    if (system_ptr_->GetLocalizer() != nullptr &&
        system_ptr_->GetLocalizer()->GetGlobalMapUpdate()) {
      auto global_map =
          ToPointCloud2(system_ptr_->GetLocalizer()->GetGlobalMap(), "map",
                        last_visualize_time_);
      global_map_pub_.publish(global_map);
    }
    if (system_ptr_->GetLocalizer() != nullptr) {
      auto submap =
          ToPointCloud2(system_ptr_->GetSubmap(), "odom", last_visualize_time_);
      submap_pub_.publish(submap);
    }

    // 系统以及初始化完成后，但是还在处理雷达消息，可视化的线程要比里程计的线程快
    if (last_visualize_time_ == system_ptr_->GetSystemTime() &&
        system_ptr_->IsSystemInit()) {
      continue;
    }
    // 系统为初始化，则不发布可视化信息
    ...
    if (has_encoder_) {
      ...
      PublishPath(encoder_path_pub_, encoder_path_, "odom",
                  last_visualize_time_, T_WE);
    }
    // 发布回环检测可视化
    PublishLoopClosure();
...
void ROS1Manager::PublishTF(const double &sensor_time) {
  ...
  geometry_msgs::TransformStamped tran_OM =
      GetTransformStamped(sensor_time, T_OtoM);
  ...
  geometry_msgs::TransformStamped tran_OB =
      GetTransformStamped(sensor_time, T_bInG);
  tran_OB.header.frame_id = "odom";
  tran_OB.child_frame_id = "robot_link";
  tf_broadcaster_->sendTransform(tran_OB);
  // 发布lidar到imu的tf信息
  geometry_msgs::TransformStamped tran_LI =
      GetTransformStamped(sensor_time, system_ptr_->GetTLidarToImu(), true);
  ...
  geometry_msgs::TransformStamped tran_BL =
      GetTransformStamped(sensor_time, system_ptr_->GetLidarToBaselink());
...
void ROS1Manager::PublishState(const double &sensor_time) {
  if (lio_odom_pub_.getNumSubscribers() != 0) {
    ...
    PoseTrans robot_pose =
        current_pose * system_ptr_->GetImuToBaselink().inverse();
...
void ROS1Manager::PublishLidar(const double &sensor_time) {
  auto cloud_lidar = ToPointCloud2(system_ptr_->GetCloudInLidarLink(),
                                   "lidar_link", sensor_time);
  cloud_lidar_pub_.publish(cloud_lidar);
  auto cloud_robot = ToPointCloud2(system_ptr_->GetCloudInRobotLink(),
                                   "robot_link", sensor_time);
  cloud_robot_pub_.publish(cloud_robot);
  auto cloud_odom =
      ToPointCloud2(system_ptr_->GetCloudInOdomLink(), "odom", sensor_time);
  cloud_odom_pub_.publish(cloud_odom);
}

void ROS1Manager::PoseTransToPoseStampedMsg(
    const PoseTrans &pose_trans, geometry_msgs::PoseStamped &pose_msg) {
...
void ROS1Manager::RosPoseToPoseTrans(const geometry_msgs::Pose &pose_msg,
...
void ROS1Manager::PoseTransToOdomMsg(const PoseTrans &pose_trans,
...
ROS1Manager::GetTransformStamped(const double timestamp,
...
ROS1Manager::ToPointCloud2(const PointCloudXYZIPtr &cloud,
...
void ROS1Manager::PublishLoopClosure() {
  if (loop_closure_pub_.getNumSubscribers() == 0)
    return;

  auto pgo = system_ptr_->GetPoseGraphOptimizer();
  if (!pgo)
    return;

  auto edges = pgo->GetLoopEdges();
  if (edges.empty())
    return;
  ...
  edge_marker.scale.x = 0.1; // 线宽
  edge_marker.color.r = 1.0;
  edge_marker.color.g = 0.4;
  edge_marker.color.b = 0.0;
  edge_marker.color.a = 0.9;
  ...
  for (const auto &edge : edges) {
    PoseTrans from_pose, to_pose;
    if (!pgo->GetOptimizedPose(edge.from_id, from_pose) ||
        !pgo->GetOptimizedPose(edge.to_id, to_pose)) {
      continue;
    }

    geometry_msgs::Point p_from, p_to;
    p_from.x = from_pose.t[0];
    p_from.y = from_pose.t[1];
    p_from.z = from_pose.t[2];
    p_to.x = to_pose.t[0];
    p_to.y = to_pose.t[1];
    p_to.z = to_pose.t[2];
    ...
    node_marker.points.push_back(p_from);
    node_marker.points.push_back(p_to);
  }

  marker_array.markers.push_back(edge_marker);
  marker_array.markers.push_back(node_marker);
  loop_closure_pub_.publish(marker_array);
}

void ROS1Manager::PublishPath(const ros::Publisher pub, nav_msgs::Path &path,
...
void ROS1Manager::MetamapsCallback(
    const robot_manager::metaset_info::ConstPtr &metamaps_msg) {
  LOG_INFO("receive metaset_info label size: {} leaf map size: {}",
           metamaps_msg->labels.size(), metamaps_msg->v_name.size());
  std::string map_dir =
      system_ptr_->GetSystemConfig()->localizer_config_.local_map_dir;
  int success_cnt = 0;
  ...
  for (auto &leaf_map : metamaps_msg->v_name) {
    ...
    for (const auto &iter : boost::filesystem::directory_iterator(
             boost::filesystem::path(leaf_node_path))) {
      // 跳过不是目录的文件
  ...
  for (auto &leaf_map : leaf_maps) {
    for (auto metamap : leaf_map.second) {
      ...
      try {
        ...
        PoseTrans T(Eigen::Quaterniond(data[6], data[3], data[4], data[5])
                        .toRotationMatrix(),
        ...
        meta_info->level = level, meta_info->x = T.t[0], meta_info->y = T.t[1],
        meta_info->name = metamap;
        ...
        ss1 << map_dir << "/" << leaf_map.first << "/" << metamap
            << "/data_trajectory.pcd";
...
void ROS1Manager::InitPoseCallback(
    const robot_manager::slam_pose::ConstPtr &init_pose_msg) {
...
void ROS1Manager::RosInitPoseCallback(
    const geometry_msgs::PoseWithCovarianceStampedConstPtr &pose_msg) {
  LOG_INFO("get init pose \n");
  // _app->set_init_pose(pose_convert(pose_msg->pose.pose), true, false, 1,
  // _app->loc_matcher_->map_name);
  PoseTrans pose;
