...
public:
  System() = delete;
  explicit System(const std::string &config_path, const std::string &slam_mode);
  ~System();
  ...
  bool IsSystemInit() const { return system_init_.load(); }

  void SetSystemInit(bool init) { system_init_.store(init); }

  const std::shared_ptr<Localizer> GetLocalizer();
  void SetInitPose(const PoseTrans &init_pose, int level = 0,
...
private:
  ...
  SLAM_MODE slam_mode_ = SLAM_MODE::MAPPING;
};
} // namespace slam