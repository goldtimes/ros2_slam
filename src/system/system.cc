...
namespace slam {
System::System(const std::string &config_path, const std::string &slam_mode)
    : config_path_(config_path), slam_mode_str(slam_mode) {
  InitConfigParams();
  if (slam_mode_str == "mapping") {
    slam_mode_ = SLAM_MODE::MAPPING;
  } else if (slam_mode_str == "slam") {
    slam_mode_ = SLAM_MODE::LOCALIZATION;
  } else if (slam_mode_str == "odometer") {
    slam_mode_ = SLAM_MODE::ODOMETER;
  }

  lidar_process_ptr_ = std::make_shared<LidarProcess>(
      system_config_ptr_->lidar_config_.lidar_type,
      system_config_ptr_->lidar_config_.use_livox_driver,
      system_config_ptr_->lidar_config_.lidar_min_range,
      system_config_ptr_->lidar_config_.lidar_max_range,
      system_config_ptr_->lidar_config_.point_filter_num,
      system_config_ptr_->frontend_config_.keep_angle_ranges,
  ...
  LOG_INFO("slam_mode: {}", slam_mode_str);
}
...
void System::AddEncoder(const Encoder &encoder) {
  ...
  while (!encoder_queue_.empty()) {
    if (encoder_queue_.back().timestamp_ - encoder_queue_.front().timestamp_ >
        1.0) {
...
void System::SetInitPose(const PoseTrans &init_pose, int level,
                         const std::string &map_id) {
  localizer_ptr_->SetInitPose(init_pose, level, map_id);
}
void System::SetMetaInfo(
    const std::map<std::string, std::vector<std::shared_ptr<MetaInfo>>>
        &meta_maps) {
...
const PoseTrans System::GetTLidarToImu() const { return T_IL; }

const PoseTrans System::GetLidarToBaselink() const { return T_BL; }

const PoseTrans System::GetImuToBaselink() const { return T_BI; }

const State System::GetCurentNavState() const {
  return front_end_ptr_->GetCurentNavState();
}

const M33D System::GetCov() const { return front_end_ptr_->GetCov(); }
