...
bool SystemConfig::LoadAndPrintConfig(const std::string &config_path) {
    try {
        ...
        frontend_config_.save_map = config["front_end"]["save_map"].as<bool>();
        frontend_config_.pcd_save_interval = config["front_end"]["pcd_save_interval"].as<int>();
        frontend_config_.map_dir = config["front_end"]["map_dir"].as<std::string>();
