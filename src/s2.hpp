   ...
   public:
    ...
    Eigen::Matrix<double, 3, 2> S2_Mx(const Eigen::Matrix<double, 2, 1> &delta) const {
        ...
        if (delta.norm() < 1e-5) {
             ...
             */
            res = -exp_delta.matrix() * SO3::hat(vec_) * A_matrix(Bu).transpose() * Bx;
        }
        return res;
    }
    ...
    void oplus(const V3D &delta, double scale = 1.0) {
        vec_ = exp(delta, scale * 0.5) * vec_;
    }
