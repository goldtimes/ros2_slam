#pragma once
#include "localizer/ceres_icp.hpp"
...
class Point_cloud_registration {
   public:
    ...
    pcl::KdTreeFLANN<PointXYZI> m_kdtree_corner_from_map;
    pcl::KdTreeFLANN<PointXYZI> m_kdtree_surf_from_map;
    ...
    // source corner, source surface, target corner, target surface
    int find_out_incremental_transfrom(PointCloudXYZIPtr in_laser_cloud_corner_from_map,
                                       PointCloudXYZIPtr in_laser_cloud_surf_from_map,
                                       pcl::KdTreeFLANN<PointXYZI> &kdtree_corner_from_map,
                                       pcl::KdTreeFLANN<PointXYZI> &kdtree_surf_from_map,
                                       PointCloudXYZIPtr laserCloudCornerStack, PointCloudXYZIPtr laserCloudSurfStack) {
        ...
        PointCloudXYZI laser_cloud_corner_from_map = *in_laser_cloud_corner_from_map;
        PointCloudXYZI laser_cloud_surf_from_map = *in_laser_cloud_surf_from_map;
        ...
        // screen_printf( "map corner num {}  surf num {} \n", laserCloudCornerFromMapNum, laserCloudSurfFromMapNum );
        ...
        if (laserCloudSurfFromMapNum > SURFACE_MIN_MAP_NUM &&
            ...
            for (iterCount = 0; iterCount < m_para_icp_max_iterations; iterCount++) {
                ...
                // laser_corner_pt_num = 0 ;
                laser_corner_pt_num = 0;
                // std::cout<<"laser serface pt num:"<< laser_surface_pt_num<<std::endl;
                for (int i = 0; i < laser_surface_pt_num; i++) {
                    ...
                    m_kdtree_surf_from_map.nearestKSearch(pointSel, plane_search_num, m_point_search_Idx,
                            ...
                            if (ICP_PLANE) {
                                ...
                                if (m_if_motion_deblur) {
                                    cost_function = ceres_icp_point2plane_mb<double>::Create(
                                        curr_point, ToV3D(laser_cloud_surf_from_map.points[m_point_search_Idx[0]]),
                                        ToV3D(
                                            laser_cloud_surf_from_map.points[m_point_search_Idx[plane_search_num / 2]]),
                                        ToV3D(
                                            laser_cloud_surf_from_map.points[m_point_search_Idx[plane_search_num - 1]]),
                                        refine_blur(pointOri.intensity, m_minimum_pt_time_stamp,
                                                    m_maximum_pt_time_stamp) *
                                            1.0,
                                        Eigen::Matrix<double, 4, 1>(m_q_w_last.w(), m_q_w_last.x(), m_q_w_last.y(),
                                                                    m_q_w_last.z()),
                                        m_t_w_last);
                                ...
                                block_id =
                                    problem.AddResidualBlock(cost_function, loss_function, m_para_buffer_incremental,
                ...
                if (residual_block_ids.size() > (size_t)m_maximum_allow_residual_block) {
                    ...
                    float threshold_to_reserve =
                        (float)m_maximum_allow_residual_block / (float)residual_block_ids.size();
                    std::cout << "threshold_to_reserve" << threshold_to_reserve << std::endl;
                    float *probability_to_drop = m_rand_float.rand_array_uniform(0, 1.0, residual_block_ids.size());
                    std::cout << "Number of residual blocks too Large, drop them to " << m_maximum_allow_residual_block
                              << std::endl;
                ...
                // If the number of residual block too Large, randomly drop some of them to guarentee the real-time
                // perfromance.
                for (size_t ii = 0; ii < 1; ii++) {
                    ...
                    // std::cout << "Inlier threshold is: " << m_inliner_ratio_threshold << endl;
                    for (unsigned int i = 0; i < residual_block_ids.size(); i++) {
                        if ((fabs(residuals[3 * i + 0]) + fabs(residuals[3 * i + 1]) + fabs(residuals[3 * i + 2])) >
                            m_inlier_threshold)  // std::min( 1.0, 10 * avr_cost )
                        {
                            // std::cout << "Remove outliers, drop id = " << (void *)residual_block_ids[i] << endl;
                            problem.RemoveResidualBlock(residual_block_ids[i]);
                        } else {
                            residual_block_ids_temp.push_back(residual_block_ids[i]);
                        }
                ...
                set_ceres_solver_bound(problem, m_para_buffer_incremental);
                ceres::Solve(options, &problem, &summary);
                if (false && m_if_motion_deblur) {
                    compute_interpolatation_rodrigue(q_w_incre, m_interpolatation_omega, m_interpolatation_theta,
            ...
            // screen_printf( "===== corner factor num {} , surf factor num {}=====\n", corner_avail_num, surf_avail_num
            // );
            if (laser_corner_pt_num != 0 && laser_surface_pt_num != 0) {
                // m_logger_common->printf( "Corner  total num {} |  use {} | rate = {} \% \r\n", laser_corner_pt_num,
                // corner_avail_num, ( corner_avail_num ) *100 / laser_corner_pt_num ); m_logger_common->printf(
                // "Surface total num {} |  use {} | rate = {} \% \r\n", laser_surface_pt_num, surf_avail_num, (
            ...
            //*( m_logger_common->get_ostream() ) << "Last R:" << m_q_w_last.toRotationMatrix().eulerAngles( 0, 1, 2
            //).transpose() * 57.3 << " ,T = " << m_t_w_last.transpose() << endl;
            //*( m_logger_common->get_ostream() ) << "Curr R:" << m_q_w_curr.toRotationMatrix().eulerAngles( 0, 1, 2
            //).transpose() * 57.3 << " ,T = " << m_t_w_curr.transpose() << endl;
            //*( m_logger_common->get_ostream() ) << "Iteration time: " << iterCount << endl;

            // m_logger_common->printf( "Motion blur = {} | ", m_if_motion_deblur );
            // m_logger_common->printf( "Cost = %.5f| inlier_thr = %.2f |blk_size = {} | corner_num = {} | surf_num = {}
            // | angle dis = %.2f | T dis = %.2f \r\n", minimize_cost, m_inlier_threshold, summary.num_residual_blocks,
    ...
    int find_out_incremental_transfrom(PointCloudXYZIPtr in_laser_cloud_corner_from_map,
        ...
        return find_out_incremental_transfrom(in_laser_cloud_corner_from_map, in_laser_cloud_surf_from_map,
    ...
    void pointAssociateToMap(PointXYZI const *const pi, PointXYZI *const po, double interpolate_s = 1.0,
                             int if_undistore = 0) {
    ...
    void pointAssociateTobeMapped(PointXYZI const *const pi, PointXYZI *const po) {
    ...
    unsigned int pointcloudAssociateToMap(PointCloudXYZI const &pc_in, PointCloudXYZI &pt_out, int if_undistore = 0) {
        ...
        for (unsigned int i = 0; i < points_size; i++) {
            pointAssociateToMap(
                ...
                refine_blur(pc_in.points[i].intensity, m_minimum_pt_time_stamp, m_maximum_pt_time_stamp), if_undistore);
        }

        return points_size;
    }

    unsigned int pointcloudAssociateTbeoMapped(PointCloudXYZI const &pc_in, PointCloudXYZI &pt_out) {
        unsigned int points_size = pc_in.points.size();
        pt_out.points.resize(points_size);
