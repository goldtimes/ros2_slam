...
bool StaticImuInit::TryInit() {
    ...
    // LOG_INFO(REDPURPLE "mean acc:{}" RESET, mean_acce.transpose());
    // LOG_INFO(REDPURPLE "mean gyro:{}" RESET, mean_gyro.transpose());
    mean_acc_ = mean_acce;
    mean_gyro_ = mean_gyro;
    // 估计重力后重新计算加速度的均值和方差
    gravity_ = -mean_acce / mean_acce.norm() * options_.gravity_norm_;
    // LOG_INFO(REDPURPLE "gravity:{}" RESET, gravity_.transpose());
    ComputeMeanAndCovDiag(init_imu_buffer_, mean_acce, cov_acc_,
                          [this](const IMU& data) { return data.acc + gravity_; });
    // LOG_INFO("after add gravity mean acc:{}", mean_acce.transpose());
    // 检查IMU噪声
