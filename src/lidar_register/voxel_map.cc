...
void OctoTree::insert(const std::vector<PointWithCov> &input_points) {
    ...
    if (layer < max_layer - 1) {
        if (temp_points.size() != 0) std::vector<PointWithCov>().swap(temp_points);
        ...
        for (size_t i = 0; i < input_points.size(); i++) {
            ...
            if (leaves[leafnum] == nullptr) {
                leaves[leafnum] = std::make_shared<OctoTree>(max_layer, layer + 1, update_size_threshes,
                                                             max_point_thresh, plane_thresh);
...
void OctoTree::initialize_tree() {
    if (all_point_num < update_size_thresh) return;
    is_initialized = true;
    new_point_num = 0;
    build_plane(temp_points);
...
void VoxelMap::pack(const std::vector<PointWithCov> &input_points) {
    ...
    for (uint i = 0; i < plsize; i++) {
        ...
        if (it == feat_map.end()) {
            ...
            if (!feat_map[k].tree->update_enable) continue;
            if (sub_it == sub_map.end()) {
                // sub_map[k] = VoxelGrid();
    ...
    for (auto &pair : sub_map) {
        if (pair.second.type == SubVoxelType::INSERT) {
            ...
            feat_map[pair.first].tree->center =
                Eigen::Vector3d((0.5 + pair.first.x) * voxel_size, (0.5 + pair.first.y) * voxel_size,
                                (0.5 + pair.first.z) * voxel_size);
