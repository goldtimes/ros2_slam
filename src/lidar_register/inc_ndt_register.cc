 ...
 * @LastEditTime: 2025-09-12 10:01:32
 * @FilePath: /fast_lvio_ws/src/lio_slam/src/lidar_register/inc_ndt_register.cc
...
IncNdtRegister::IncNdtRegister(
  ...
  min_effective_pts_ =
      system_config->frontend_config_.ndt_config.min_effective_pts;
  min_pts_in_voxel_ =
      system_config->frontend_config_.ndt_config.min_pts_in_voxel;
  max_pts_in_voxel_ =
      system_config->frontend_config_.ndt_config.max_pts_in_voxel;
  res_outlier_thresh_ =
      system_config->frontend_config_.ndt_config.res_outlier_thresh;
  eps_ = system_config->frontend_config_.ndt_config.eps;
  calib_lidar2imu_ = system_config->frontend_config_.calib_lidar2imu;

  ndt_ptr_ = std::make_shared<IncNdt>(voxel_size_, near_search_, max_capacity_,
                                      min_effective_pts_, min_pts_in_voxel_,
                                      max_pts_in_voxel_, res_outlier_thresh_,
                                      eps_, calib_lidar2imu_);
  // 设置雷达损失函数
...
IncNdtRegister::~IncNdtRegister() { LOG_INFO("~IncNdtRegister"); }

bool IncNdtRegister::InitMap(PointCloudXYZIPtr &cloud_lidar,
...
bool IncNdtRegister::Align(PointCloudXYZIPtr &cloud_lidar,
                           std::shared_ptr<IESKF> /*kf_ptr_*/) {
  // transform cloud_lidar to body
  auto cloud_body = TransformLidarOMP(cloud_lidar, system_config_->lidar2imu_.R,
...
void IncNdtRegister::UpdateMap() {
  ...
  if (delta_pose.t.norm() > 0.5 ||
      delta_pose.RPY().norm() > (10.0 / 180.0 * M_PI)) {
    auto cloud_world =
        TransformLidarOMP(ndt_ptr_->source_, current_pose.R, current_pose.t);
...
PointCloudXYZIPtr IncNdtRegister::GetSubmap() {
  ...
  // 从NDT体素地图中提取所有已估计体素的均值点，构建submap
