...
private:
  inline void update_worst_unrolled() {
    float d0 = dist2_[0], d1 = dist2_[1], d2 = dist2_[2], d3 = dist2_[3],
          d4 = dist2_[4];
...
template <typename Point> class OctVox {
public:
  ...
  bool getPoint(const uint8_t local_idx, Point &pt) const {
    if (counts_[local_idx] == UNINIT_MASK)
      return false;
...
template <typename Point, typename Scalar> class OctVoxMap {
public:
  ...
  OctVoxMap() {
    ...
    for (std::size_t i = 0; i < flat_search_order_offsets.size(); i++) {
      ...
      flat_search_ptrs_.push_back(
          const_cast<uint8_t *>(flat_search_order.data() + start));
    }
...
private:
  ...
  const KEY nearby_grids_[19] = {
      KEY(0, 0, 0),  KEY(-1, -1, 0), KEY(-1, 0, 0), KEY(-1, 1, 0),
      KEY(0, -1, 0), KEY(0, 1, 0),   KEY(1, -1, 0), KEY(1, 0, 0),
      KEY(1, 1, 0),  KEY(0, 0, -1),  KEY(1, 0, -1), KEY(-1, 0, -1),
      KEY(0, 1, -1), KEY(0, -1, -1), KEY(0, 0, 1),  KEY(1, 0, 1),
      KEY(-1, 0, 1), KEY(0, 1, 1),   KEY(0, -1, 1)};
...
void OctVoxMap<Point, Scalar>::insert(const Points &cloud_world) {
  if (reset_map_) {
    ...
    if (reset_map_count_ > 0) {
      std::cout << "OctVoxMap::insert skip: reset_map_count_ = "
                << reset_map_count_ << std::endl;
      return;
    }
    reset_map_ = false;
  }
  ...
  for (auto &pt : cloud_world) {
    KEY fine_key =
        (pt * sub_inv_resolution_).array().floor().template cast<int>();
    KEY key;
    key[0] = fine_key[0] >> 1;
    key[1] = fine_key[1] >> 1;
    key[2] = fine_key[2] >> 1;
    ...
    if (iter == grids_.end()) {
      data_.emplace_front(std::piecewise_construct, std::forward_as_tuple(key),
...
template <typename Point, typename Scalar>
void OctVoxMap<Point, Scalar>::getTopK(const Point &point,
                                       KNNHeapType &top_K) const {
  const KEY fine_key =
      (point * sub_inv_resolution_).array().floor().template cast<int>();
  KEY key;
  key[0] = fine_key[0] >> 1;
  key[1] = fine_key[1] >> 1;
  key[2] = fine_key[2] >> 1;
  ...
  for (int group_idx = 0; group_idx < group_idx_max_; ++group_idx) {
    ...
    while (group_it < group_end) {
      ...
      KEY delta_key =
          mirror_axis.cwiseProduct(HKNN_neighbor_voxel[neighbor_idx]);
...
template <typename Point, typename Scalar>
void OctVoxMap<Point, Scalar>::getTopK_VN(const Point &point,
                                          KNNHeapType &top_K) const {
...
void OctVoxMap<Point, Scalar>::saveMap() const {
  ...
  std::string filename = "/tmp/OctVoxMap.pcd";
  ...
  if (result == 0) {
    std::cout << "Successfully saved " << cloud->points.size() << " points to "
