...
P2PlaneRegister::P2PlaneRegister(
  ...
  map_resolution =
      system_config->frontend_config_.p2plane_config.map_resolution;
  ...
  LOG_INFO("map resolution: {}, cube_len:{}, det_range:{}, move_thresh:{}",
...
bool P2PlaneRegister::InitMap(PointCloudXYZIPtr &cloud_lidar,
  ...
  if (first_frame_) {
    ...
    LOG_INFO("Build Map Size:{}, cloud  size:{}", m_ikdtree->size(),
...
void P2PlaneRegister::TrimCloud() {
  ...
  const auto current_state = kf_ptr_->GetState();
  V3D pos_lidar = PoseTrans(current_state.rot, current_state.pos) *
                  system_config_->lidar2imu_.t;
  // 初始化立方体的范围
  if (!m_local_map.initialized) {
    for (int i = 0; i < 3; ++i) {
      m_local_map.local_map_corner.vertex_min[i] =
          pos_lidar[i] - cube_len / 2.0;
      m_local_map.local_map_corner.vertex_max[i] =
          pos_lidar[i] + cube_len / 2.0;
    }
  ...
  for (int i = 0; i < 3; ++i) {
    dist_to_map_edge[i][0] =
        std::fabs(pos_lidar[i] - m_local_map.local_map_corner.vertex_min[i]);
    dist_to_map_edge[i][1] =
        std::fabs(pos_lidar[i] - m_local_map.local_map_corner.vertex_max[i]);
    // 小于阈值就需要移动地图
    if (dist_to_map_edge[i][0] <= det_thresh ||
        dist_to_map_edge[i][1] <= det_thresh) {
      need_move = true;
    }
  }
  if (!need_move) {
  ...
  float move_dist =
      std::max((cube_len - 2.0 * move_thresh * det_thresh) * 0.5 * 0.9,
               double(det_range * (move_thresh - 1)));
  for (int i = 0; i < 3; ++i) {
    ...
    if (dist_to_map_edge[i][0] <= det_thresh) {
      ...
      temp_corner.vertex_min[i] =
          m_local_map.local_map_corner.vertex_max[i] - move_dist;
    ...
    if (dist_to_map_edge[i][1] <= det_thresh) {
      ...
      temp_corner.vertex_max[i] =
          m_local_map.local_map_corner.vertex_min[i] + move_dist;
      m_local_map.cub_to_rm.push_back(temp_corner);
    }
  }
...
void P2PlaneRegister::IncreMap() {
  ...
  for (int i = 0; i < cloud_size; ++i) {
    ...
    mid_point.x =
        std::floor(cloud_world->points[i].x / map_resolution) * map_resolution +
        0.5 * map_resolution;
    mid_point.y =
        std::floor(cloud_world->points[i].y / map_resolution) * map_resolution +
        0.5 * map_resolution;
    mid_point.z =
        std::floor(cloud_world->points[i].z / map_resolution) * map_resolution +
        0.5 * map_resolution;
    ...
    for (int readd_i = 0; readd_i < 5; readd_i++) {
      ...
      if (points_near.size() < static_cast<size_t>(5))
        break;
...
void P2PlaneRegister::UpdateLidarFunc(State &nav_state,
...
#pragma omp parallel for
  ...
  for (int i = 0; i < size; ++i) {
    ...
    // auto pt_world_test =
    //     current_state.r_wi * (current_state.r_il * pt_lidar +
    //     current_state.t_il) + current_state.t_wi;
    // LOG_INFO("pt_world:{}, pt_world_test:{}", pt_world.transpose(),
    // pt_world_test.transpose());
    ...
    if (EstimatePlane(points_near, 0.1, plane_coeff)) {
      double pd2 = plane_coeff(0) * point_world.x +
  ...
  for (int i = 0; i < effect_feat_num; ++i) {
    ...
    Eigen::Matrix<double, 1, 3> dres_dr =
        ...
        Sophus::SO3d::hat(current_state.rot_ext * pt_lidar +
                          current_state.pos_ext);
    // 残差对平移的雅可比矩阵
...
bool P2PlaneRegister::EstimatePlane(const PointVec &points, double thresh,
                                    Eigen::Vector4d &plane_coeff) {
  ...
  for (size_t j = 0; j < points.size(); j++) {
    if (std::fabs(plane_coeff(0) * points[j].x + plane_coeff(1) * points[j].y +
                  plane_coeff(2) * points[j].z + plane_coeff(3)) > thresh) {
      return false;
    }
  }
  return true;
}
void P2PlaneRegister::UpdateMap() { IncreMap(); }
...
void P2PlaneRegister::CacheData() {}
void P2PlaneRegister::SaveMap() {}
} // namespace slam